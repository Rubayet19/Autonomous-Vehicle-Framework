...
# from rclpy.qos import QosProfile, QoSReliabilityPolicy, QoSHistoryPolicy, QoSDepthPolicy
import cv2
import os
import numpy as np
...
class ImageStitching(Node):
    ...
    def __init__(self):
        ...
        #     history=QoSHistoryPolicy.RMW_QOS_HISTORY_SYSTEM_DEFAULT,  
        #     depth=QoSDepthPolicy.RMW_QOS_DEPTH_SYSTEM_DEFAULT
        # )
        ...
        # camera_link_front = os.readlink('/dev/front_cam')
        # camera_link_left = os.readlink('/dev/left_cam')
        # camera_link_right = os.readlink('~/dev/right_cam')

        self.front_fisheye = cv2.VideoCapture('front_cam', cv2.CAP_V4L2) # Fisheye Front
        self.left_fisheye = cv2.VideoCapture('left_cam', cv2.CAP_V4L2) # Left
        self.right_fisheye = cv2.VideoCapture('right_cam', cv2.CAP_V4L2) # Right
        ...
        self.left_image = np.empty([240, 320, 3]).astype('uint8')
        self.right_image = np.empty([240, 320, 3]).astype('uint8')
        ...
        self.stitched_image = np.empty([720, 1080, 3]).astype('uint8') #np.empty([1920, 960])
    ...
    def stitching_callback(self):
        try:
            ...
            merged_frames = cv2.resize(merged_frames, dsize=(1080,480), interpolation=cv2.INTER_LINEAR)
